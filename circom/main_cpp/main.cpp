...
void Verifier_7_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Verifier_7_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 5284;}

uint get_number_of_components() {return 48;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 5114;}
...
Fr_sub(&expaux[3],&signalValues[mySignalStart + 4],&circuitConstants[2]); // line circom 17
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[3]); // line circom 17
...
uint csoffset = mySignalStart+1133;
for (uint i = 0; i < 1; i++) {
...
uint csoffset = mySignalStart+5239;
...
uint csoffset = mySignalStart+2249;
uint aux_dimensions[1] = {4};
for (uint i = 0; i < 4; i++) {
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 3],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 55
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[8]); // line circom 71
while(Fr_isTrue(&expaux[0])){
{
...
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 11);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 7)]].signalStart + 0]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 7)]].signalStart + 1]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
MultiMimc7_4_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 11)]].signalStart + 0]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Node_3_run(mySubcomponents[cmp_index_ref],ctx);
...
Verifier_7_create(1,0,ctx,"main",0);
Verifier_7_run(0,ctx);
}
